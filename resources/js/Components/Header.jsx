                        ...
                        <Dropdown>
                            <Dropdown.Trigger>
                                <span className="inline-flex rounded-md">
                                    <button
                                        type="button"
                                        className="inline-flex items-center gap-2 rounded-xl border border-transparent bg-transparent py-1 sm:py-1.5 pe-1 sm:pe-2 ps-1 text-xs sm:text-sm font-semibold text-slate-200 transition duration-150 ease-in-out hover:text-white focus:outline-none dark:text-slate-300 dark:hover:text-slate-100"
                                    >
                                        {/* User Photo / Initial */}
                                        {user?.photo ? (
                                            <img
                                                src={user.photo}
                                                alt={user.name}
                                                className="h-7 w-7 sm:h-8 sm:w-8 rounded-full object-cover border border-slate-600"
                                            />
                                        ) : (
                                            <div className="flex h-7 w-7 sm:h-8 sm:w-8 items-center justify-center rounded-full bg-indigo-500/20 text-[10px] sm:text-xs font-bold text-indigo-300">
                                                {user?.name
                                                    ? user.name
                                                          .charAt(0)
                                                          .toUpperCase()
                                        ...
                                        {/* Dropdown Arrow */}
                                        <svg
                                            className="-me-0.5 h-3.5 w-3.5 sm:h-4 sm:w-4 text-slate-400"
