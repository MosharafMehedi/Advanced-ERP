...
export default function Index({ users, filters }) {
    ...
    return (
        <AuthenticatedLayout
            header={<h2 className="text-xl font-bold tracking-tight text-slate-800 dark:text-slate-200">User Masterlist</h2>}
        >
                ...
                <div className="overflow-x-auto rounded-xl border border-slate-100 dark:border-slate-800">
                    <table className="w-full text-sm text-left text-slate-500 dark:text-slate-400 whitespace-nowrap">
                        <thead className="text-xs text-slate-700 dark:text-slate-300 uppercase bg-slate-50 dark:bg-slate-950/40 border-b border-slate-200 dark:border-slate-800">
                            <tr>
                                <th className="px-6 py-4 font-bold tracking-wider">Emp ID / Name</th>
                                <th className="px-6 py-4 font-bold tracking-wider">Contact Info</th>
                                <th className="px-6 py-4 font-bold tracking-wider">Dept & Designation</th>
                            ...
                            {users.data.map((user) => (
                                <tr key={user.id} className="hover:bg-slate-50/40 dark:hover:bg-slate-800/20 transition-colors">
                                    <td className="px-6 py-4">
                                        ...
                                        <div className="text-xs text-slate-400 font-semibold mt-0.5">{user.employee_id || 'N/A'}</div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className="text-slate-700 dark:text-slate-300 font-medium">{user.email}</div>
                                        <div className="text-xs text-slate-400 mt-0.5">{user.phone || 'N/A'}</div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className="text-slate-700 dark:text-slate-300 font-semibold">{user.designation || 'N/A'}</div>
                                        <div className="text-xs text-indigo-600 dark:text-indigo-400 font-bold mt-0.5">{user.department || 'N/A'}</div>
                                    </td>
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <div className="flex justify-end gap-1.5">
                                            <Link 
                                                ...
                                                className="p-2 text-slate-500 hover:text-indigo-600 dark:hover:text-indigo-400 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 transition-all"
                                            >
                                                <FiEdit2 className="h-4 w-4" />
                                            </Link>
                                            <button 
                                                onClick={() => handleDelete(user.id)} 
                                                className="p-2 text-slate-500 hover:text-rose-600 dark:hover:text-rose-400 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 transition-all"
                                            >
                            ...
                            {users.links.map((link, index) => {
                                ...
                                return (
                                    <button
                                        key={index}
                                        disabled={!link.url}
                                        onClick={() => router.get(link.url, { search: search }, { preserveState: true, preserveScroll: true })}
                                    ...
                                    >
                                        {isPrevious ? <FiChevronLeft className="h-4 w-4" /> : isNext ? <FiChevronRight className="h-4 w-4" /> : <span dangerouslySetInnerHTML={{ __html: link.label }} />}
                                    </button>
                                );
                            })}
                        </div>
